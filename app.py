...
# ---------------------- Car Value Estimator ----------------------
def calculate_car_value_over_time(initial_value, depreciation_rate, months):
    values = []
    value = initial_value
    for _ in range(months):
        values.append(value)
        value *= (1 - depreciation_rate / 100 / 12)
    return values
...
if loan_type == "Car Loan":
    depreciation_rate = st.number_input("Estimated Depreciation Rate (% per year)", min_value=0.0, max_value=30.0, value=15.0, step=0.5)
